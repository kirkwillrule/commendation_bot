...
def setup(bot): # Create a bot instance
    ...
    @bot.command()
    async def thanks_one(ctx, thankee: discord.Member):
        thanker = ctx.author
    ...
    async def thanks(ctx, *args):
        ...
        for arg in args:
            thankee = None
            # Run in parallel so we're not waiting for a single thankee to accept a role
            thankings.append(thank_single(ctx, arg, valid_thankies, invalid_thankies))

        await asyncio.gather(*thankings)    
    ...
    async def thank_single(ctx, arg, thanker, thankee):
        thanker = ctx.author
